    ...
    public void orderStatus(MapMessage mapMessage) {
        ...
        try {
            if (Arrays.asList(arr).contains(Integer.parseInt(mapMessage.getString("code")))) {
                ...
                System.out.println(sellerBrief.getAutoStatus());
                if (sellerBrief.getAutoStatus().equals(1) && mapMessage.getString("code").equals("13")) {
                    System.out.println(orderService.gen(orderMain.getOrderId()));
                    //发送短信内容MQ
